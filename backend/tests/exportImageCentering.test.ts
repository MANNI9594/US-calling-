...
describe('evidence image sizing and centering in the Master export', () => {
  it('a placed image is smaller than the full column/row and centered within it (not edge-to-edge, not top-left anchored, never overflowing the column) — exact pixel sizing via ext, the one proven-accurate approach', async () => {
    ...
    // Never overflows the column/row — this is the core "fits properly"
    // guarantee, verified against the real column width used in the
    // actual reference workbook, not a made-up number.
    expect(placement.width).toBeLessThan(fullColumnWidthPx);
    expect(placement.height).toBeLessThan(fullRowHeightPx);
    expect(placement.width).toBeLessThanOrEqual(maxImageWidthPx + 0.01);
    expect(placement.height).toBeLessThanOrEqual(maxImageHeightPx + 0.01);
    ...
    // Confirms the EXACT requested pixel size survives a real write+read
    // round trip — this is what guarantees "fits properly, never outside
    // the column," since ext specifies size directly rather than deriving
    // it from an estimated column-width conversion (which was tried and
    // found to shrink images unpredictably in real use).
