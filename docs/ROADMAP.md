...
## Scroll position preserved across edits (both VECS List and US Calling List)

User reported a real annoyance: saving one cell edit (e.g. ETA) triggered a full table reload that reset horizontal/vertical scroll back to the top-left, forcing a re-scroll to reach ETD or any other far-right column for the next edit. Considered the user's own suggested fix (only refresh when Edit Mode turns off) but proposed an alternative instead: preserve scroll position across the existing per-edit refresh, so data stays live (teammates' concurrent edits still appear promptly, sort order still updates immediately) without ever losing your place. Implemented by capturing and restoring `scrollLeft`/`scrollTop` on the table's scroll container around every reload, and skipping the "Loading…" placeholder flash for anything other than the true first page load (tab switches and initial load still show it, since those are deliberate context changes where a brief loading state is expected and scroll position is irrelevant). Applied identically to both VECS List (`loadVessels`) and US Calling List (`loadEntries`).

## Service Fees highlight rule corrected: YES or NO (not just YES)

User clarified the intent was never "highlight when Applicable" specifically — it's "highlight whenever a definitive decision has actually been made," covering both YES and NO, leaving Unknown/NA unhighlighted (not yet evaluated). Updated `serviceFeesYellowStyle()` accordingly, and — new in this pass — the Service Fees cell itself now also turns yellow (previously only Summer DWT/TEU did). Applied consistently across the portal table, the vessel detail modal, and the exported Master XLSX (columns 5 and 13).

## Major navigation cleanup + inline Master import on VECS List + rename to ENOA/D List

**Navigation simplified from 6 links to 3** everywhere: VECS List | ENOA/D List | Settings. Team, Evidence Review, and Logout moved into a new Settings hub page (`settings.html`) rather than living as separate top-level nav items. The old standalone Import Master Workbook page was **removed entirely** (explicit user decision, not just unlinked) — its Backfill Missing Fields, Clean Up Stale Flags, and Master Import History tools moved to Settings, since they're occasional maintenance actions rather than daily workflow.

**VECS List gained its own inline Master Import widget** — a collapsible "▸ Import Master Workbook" section (collapsed by default) with the exact same dropzone → preview → commit flow the old standalone page had, calling the same backend endpoints (`/api/import/master-workbook/preview` and `/commit`) rather than duplicating any business logic. This means Master import is now available directly from the page it affects, without navigating away.

**"US Calling List" renamed to "ENOA/D List"** throughout — nav label, page title, headings, and internal prose/dialog text (e.g. "Add Vessel to ENOA/D List", the delete confirmation). The underlying file/route (`us-calling-list.html`) is unchanged, matching how "VECS List" was handled earlier (rename the label, not the URL, to avoid breaking anything).

**Filters made collapsible on both list pages**, addressing the "too crowded" complaint directly: core actions (search, Select All/Clear Selection, Remove/Delete Selected, Undo/Redo) stay always visible since they're used constantly; filter chips, sort dropdown, the Columns picker (VECS List), and the ETA/ETD date-range filters (ENOA/D List) now live behind a "▸ Filters & Sort" toggle, collapsed by default.

**Not yet tested live** — next actions: (1) confirm the new 3-link nav works correctly across all pages and Settings correctly links to Team/Evidence Review/Logout, (2) try the inline Master import widget on VECS List end-to-end (drop a file, preview, confirm), (3) confirm the collapsible filter sections expand/collapse correctly on both pages and that filtering still works once expanded.

## Date pickers for inline ETA/ETD editing + dynamic learned suggestions

**Date pickers replace typed dates in Edit Mode**, on both VECS List and ENOA/D List — double-clicking an ETA/ETD cell now opens a native calendar picker instead of a text box expecting DD-MM-YYYY. This directly sidesteps the entire day/month ambiguity class of bug investigated earlier this session (a native date picker cannot produce an invalid or ambiguous date). The picker's ISO value is converted to/from the app's DD-MM-YYYY convention automatically (`ddMmYyyyToIso`/`isoToDdMmYyyy`), so the stored raw value and everything displayed in the table remain in DD-MM-YYYY as always — only the *editing* experience changed, not the data format. If an existing value happens to be unparseable (a pre-existing malformed date), the picker simply starts blank rather than erroring, so it doesn't block editing.
...
## Missing evidence shows a dash, export images resized and centered, Edit Vessel modal verified

**Missing evidence now shows "—"** instead of "No image" (portal) or a truly blank cell (export) — consistent with how every other empty field in the app displays, in both places.

**Export evidence images fixed for real, not guessed at.** A third round on image sizing: earlier fixes computed a *maximum* size but always anchored images at the cell's top-left corner, sized close to the full column width — which looked cramped/edge-to-edge, not "centered in the column." This time: (1) target size reduced to ~75% of the available column/row space rather than ~95%, and (2) images are now genuinely **centered** within their cell using ExcelJS's fractional column/row anchor positioning, not just top-left placement. Both the shrink-and-center math and ExcelJS's actual support for fractional anchors were verified with real, executed round-trip tests before being trusted — the fractional-anchor mechanism specifically, since assuming ExcelJS behavior without testing is exactly what caused the entire-sheet-turned-yellow bug fixed earlier this session. New test: `tests/exportImageCentering.test.ts`.

**Edit Vessel modal** — clicking a vessel name on VECS List now opens a fully editable form matching Add Vessel's exact field set and interaction pattern (dropdowns for Ballast/Loaded and Service Fees, dynamic learned suggestions for Owner/Operator/Built Location/Port, native ETA/ETD date pickers, and the same click/drag/paste evidence upload), pre-filled with the vessel's current values. Saving calls `PATCH /api/vessels/:id` for permanent-profile fields and `PATCH /api/vessels/:id/operational` for Port/ETA/ETD/Ballast, in sequence, with distinct error handling at each step so a failure in one doesn't silently swallow the other. This was found to already be fully built (apparently from earlier in this long session) rather than needing to be built from scratch — verified complete and correct, and a genuine duplicate declaration of two helper functions (`ddMmYyyyToIso`/`isoToDdMmYyyy`, defined twice in the file) was found and cleaned up along the way.

**Not yet tested live** — next actions: (1) confirm empty-evidence vessels show "—" in both the portal and a fresh export, (2) download the Master export and confirm images now sit visibly smaller and centered within their column rather than edge-to-edge, (3) click a vessel name on VECS List and confirm the full edit form opens pre-filled, correctly saves changes, and correctly attaches a photo via all three upload methods.

## Add Vessel redesigned as a wide grid + Apply confirmation now shows ETD
...
## KPI bar on VECS List

Four small stat cards added above the Active/Archived tabs, computed from whichever tab is currently selected: Chinese-Built Vessels (reuses the same detection logic as the yellow highlight, so the two can never disagree), Service Fees: Yes, Service Fees: No, and Bridge Letter on File (any vessel with a non-empty Bridge Letter value). Recomputed automatically every time the vessel list reloads.

## MAJOR FEATURE: US Calling (third list) + unified cross-list "Departed" model

This was built carefully and incrementally, at the user's explicit request to "take it slow, no mistakes" — every reader was tested against the user's real uploaded file before anything was built on top of it, and two real bugs were found and fixed along the way (see below) rather than discovered later in production.

### The pipeline
Three lists now exist, matching the user's real-world workflow: **US Calling** (new — a vessel is added the moment it's known to be calling a US port) → **ENOA/D List** (once its ENOA is received, real ETA/ETD/Port are recorded) → **VECS List** (the master record, synced from ENOA/D List). Named "US Calling Tracker" internally in the backend code specifically to avoid colliding with ENOA/D List's own internal name (`UsCallingListEntry`), which predates the page being renamed from "US Calling List" to "ENOA/D List" earlier in this project.

### US Calling — the new list
19 columns, all confirmed against the user's real uploaded file (`Book1.xlsx`), not assumed: Vessel Name, Tech, ETA, ETD, Port, Certificate Status, Qualship 21 Expiry Date, E-NOA sent for review, US Calling Message Sent, Received from vessel, Reviewed or not, two Ballast Water Report columns, Bio-fouling Management Plan, Marine Invasive Species Program form, Flag, two Checklist columns, Remark.

- **ETA/ETD** are true date-only fields (native date pickers) — confirmed with the user that "In Port" (seen in early sample data) is not a real value here.
- **E-NOA sent for review** is a true fixed dropdown (Yes/No).
- **Flag and Port** use the same dynamic, learned-from-real-usage suggestion pattern as the rest of the app (`GET /api/us-calling-tracker/distinct-values`).
- **Every other date-ish column** (Qualship 21 Expiry, US Calling Message Sent, both Ballast Water columns, etc.) holds either a real completion date OR a status like "NA"/"-"/"Not Eligible" depending on real data — confirmed against actual rows, not assumed. These are free-text fields with a shared quick-pick datalist (`NA`, `-`, `Not Eligible`) that still accepts a typed real date.
- **Wrapped headers**, not the app-wide `nowrap` convention — a deliberate, explicit exception for this one page, since 19 columns with genuinely long names (e.g. "Marine Invasive Species Program Annual Vessel Reporting Form...") would otherwise force enormous horizontal scrolling.
- **"Not in VECS List" yellow highlight + filter** — every entry's vessel name is cross-referenced against VECS List by normalized name (`inVecs` flag, computed server-side); unmatched names get the same yellow highlight styling used elsewhere in the app, plus a dedicated filter chip.
- **Add Vessel** is a wide-grid modal (short fields paired, long fields full-width — matching VECS List's redesigned layout) covering all 19 fields, but **deliberately without** VECS List's "search archive first, offer to restore" step — per explicit clarification, that concept is exclusive to VECS List, since only VECS has an archived/recoverable vessel concept.
- Column visibility toggle, collapsible import/filters, undo/redo, live sync, "download respects the current filter" — full feature parity with the other two lists.

### Unified "Departed" model across all three lists
Previously, "Departed" only existed on VECS List/ENOA-D List and was stored directly on the VECS `Vessel` record. This was rebuilt as a shared, standalone `DepartedVesselMarker` table keyed by normalized vessel name — deliberately **not** tied to a specific Vessel row, since a vessel can be marked Departed from US Calling or ENOA/D List *before* it ever reaches VECS List.

- **VECS List's "Remove Selected" is now labeled "Departed"** — same underlying archive action, now also records the shared marker.
- **ENOA/D List and US Calling's "Departed" action** deletes the row from that list and records the same shared marker.
- **The badge and a "Departed" filter chip appear on all three lists** wherever that vessel currently exists — marking departed from any one list makes it visible everywhere else the vessel is tracked.
- **Real Undo/Redo for Departed actions**, not just field edits — the undo stack now supports a second entry type alongside simple field edits. For VECS List (where archiving doesn't change the vessel's ID), Undo/Redo simply toggles restore/archive on the same ID. For ENOA/D List and US Calling (where "Departed" deletes the row), the full row is snapshotted before deletion so Undo can recreate an equivalent row — and since that recreation gets a **new** ID, the undo entry is updated in place with that new ID so a subsequent Redo knows what to re-depart.
- **Auto-clears on fresh data or restore** — if a vessel marked Departed later gets fresh operational data applied (via any upload/apply path) or gets manually restored, the marker clears automatically everywhere, consistent with the principle established earlier this session for stale flags.

### Two real bugs found and fixed while building this (not guessed at — reproduced and proven)
1. **Rich-text cell values silently broke header matching.** ExcelJS represented one real header ("Marine Invasive Species Program...") as a rich-text object rather than a plain string — `String(cell.value)` on that produced the literal text `"[object Object]"`, not the header, which would have silently failed to match that column. Found via testing against the real file, confirmed with a minimal reproduction, then fixed with a new shared `extractCellText()` utility applied to **all three** Excel readers in the codebase (not just the one that triggered it), since the same vulnerable pattern existed in all three.
2. **A non-breaking space in another real header** ("...Checklist sent␣to vessel") was confirmed to already be handled correctly by the existing header-normalization utility (verified empirically, not assumed) — extracted into a shared `normalizeHeaderText()` utility so both existing readers and the new one use the exact same proven logic.

### Also retrofitted onto the two existing lists in this batch
- **"Download respects the current filter"** — VECS List and ENOA/D List's download buttons now export only what's currently filtered/searched on screen (by passing the visible row IDs to the export endpoint), with the full list still one click away by clearing filters first. Built into US Calling from the start.

**Not yet tested live** — this is the largest single feature added in this project's history and genuinely needs a full live pass before being trusted: (1) deploy and add a vessel to US Calling, confirm it shows the "Not in VECS" badge, (2) apply it through to ENOA/D List and VECS List and confirm the badge clears, (3) mark a vessel Departed from each of the three lists in turn and confirm the badge/filter appears correctly on the other two, (4) test Undo immediately after each Departed action and confirm the row/vessel genuinely comes back, (5) test Redo after that Undo, (6) import the real Book1.xlsx file through the UI (not just the automated reader test) and confirm every column lands correctly, (7) confirm the downloaded export matches the original file's column order and headers.

## Fixed: Railway deployment failure caused by my own earlier mistake

My previous fix for the `--accept-data-loss` error was wrong and actually broke the build entirely. I'd assumed this was a simple Railway buildpack deployment and added `postinstall`/`prestart` scripts to `package.json` to run Prisma commands automatically — but the real deploy log revealed this project actually uses a hand-written multi-stage **Dockerfile** (`backend/Dockerfile`), which already had its own carefully-ordered Prisma steps. My `postinstall: prisma generate` ran during `npm install`, which happens in the Dockerfile *before* `prisma/schema.prisma` is even copied into the build (deliberately, for Docker layer caching) — so it failed with "Could not find Prisma Schema." My `prestart` addition was also pointless, since this Dockerfile's `CMD` runs `node dist/index.js` directly, never going through npm's `start` script lifecycle at all.

**Reverted** both package.json additions back to their original state, and instead made the one-line, correctly-targeted fix: added `--accept-data-loss` to the Dockerfile's existing `CMD` line, which already ran `prisma db push` in exactly the right place (after the schema is copied in, at container startup). Documented directly in the Dockerfile why this flag is a permanent fixture, not a one-time bypass: since this deployment path has no CLI access for a human to approve a schema change case-by-case, `prisma db push` would otherwise hard-block the entire app from starting on any future schema change that removes or narrows a column — even a brand-new, empty one with nothing real to lose. Care about genuinely risky schema changes needs to happen when editing `schema.prisma`, not at deploy time.

## Fixed: stale Departed markers survived re-adding a vessel

Real bug, found via a real scenario the user described: a vessel's ENOA gets sent by mistake, gets removed from ENOA/D List via "Departed" (which correctly flags it everywhere), then gets added back once the mistake is caught — but the vessel was still showing as "Departed" on the other lists afterward, even though it was actively, visibly present and being tracked again.

**Root cause**: the auto-clear-on-fresh-data logic only ever covered the *apply/import* commit path (`runCommitTransaction`), not a plain manual "Add Vessel" create, and US Calling's file-import path used its own separate upsert logic that was never wired to auto-clear at all. So re-adding a vessel by hand (or via a fresh import) never touched the stale marker from an earlier "Departed" action.
...
## REVERTED: the copy-paste anchor-type fix broke image sizing, and didn't even fix the original problem

The previous change (switching evidence images to `twoCellAnchor`/`editAs="twoCell"` so they'd travel with a normal cell-range copy-paste) was reverted after real-world testing showed two problems: the images came out much smaller than intended, and — worse — the user confirmed copy-paste still didn't carry the images anyway, so the change caused a regression without even achieving its goal.

**Why the sizing broke**: `twoCellAnchor` derives an image's size from the span between two cell corners, which requires converting a target pixel size into a *fraction of the column's actual rendered width*. That conversion depends on accurately estimating how many pixels the column renders as — an estimate that turned out to be off, silently shrinking every image below its intended size.

**Reverted** back to the exact pixel-size approach (`ext`) that was previously confirmed working and "perfect" — this guarantees images render at exactly the intended size, never overflowing the column, verified via the same real round-trip test used before (restored, not just reasoned about). The lesson here: an unverified fix for one problem (copy-paste) is not worth risking a proven, working guarantee (correct sizing) — especially when the unverified fix didn't even solve what it was meant to solve. "Copy as Picture" remains the one reliably-guaranteed method for getting evidence images into a pasted email body, since it doesn't depend on Excel's image anchor type at all.
